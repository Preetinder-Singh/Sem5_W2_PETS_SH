...
/*******
 * <p>
 * Title: TaskSpecific Class
 * </p>
 * 
 * <p>
 * Description: A JavaFX application: This controller class established the user
 * interface for Tasks Tab objects by inheriting the ListItem and building upon
 * it.
 * </p>
 * 
 * <p>
 * Copyright: Lynn Robert Carter © 2019-05-11
 * </p>
 * 
 * @author Lynn Robert Carter
 * @author Preetinder
 * 
 * @version 1.00 2019-08-10 
 * 
 */
...
// ---------------------------------------------------------------------------------------------
// These attributes enable us to hide the details of the tab height and the
// height and width of
...
// ---------------------------------------------------------------------------------------------
// The following are the GUI objects that make up the Task specific elements of
// the
// user interface
	...
	private ComboBox<String> chooseUsedEffortCategory = new ComboBox<String>(
			FXCollections.observableArrayList(effortCategoryList));
	...
	protected TableView<ECUsedItem> theProducedDetailList = new TableView<ECUsedItem>();
	private TableColumn<ECUsedItem, String> theProducedNameColumn = new TableColumn<>("Produced Artifact Names");
...
// ---------------------------------------------------------------------------------------------
// The following are the list objects that make it possible to display the GUI
// elements needed
...
// This is the Task specific Detail list
	...
	/**********
	 * This constructor establishes the base ListItem and then initializes the Task
	 * specific attributes for the application.
	 * 
	 * @param g The Group link is used to establish the list of GUI elements for
	 *          this tab
...
// Setup the title and the ComboBox which selects one of the Effort Category
// entries to
// us from which the list of artifacts is defined
		...
		chooseUsedEffortCategory.setOnAction((event) -> {
			chooseUsedEffortCategory();
		});
...
// This ComboBox specifies available artifacts. No action is performed when one
// is selected
// The user must press the insert button to cause the selected item to be added
// to the list
		setupComboBoxUI(theUsedArtifact, 245, 700 + xOffset, 218 + yOffset);

// This button adds the currently selected Artifact to the list of Artifacts
// used
		setupButtonUI(btnInsertUsed, "Arial", 14, 245, Pos.BASELINE_CENTER, 700 + xOffset, 250 + yOffset);
		btnInsertUsed.setOnAction((event) -> {
			insertUsedArtifact();
		});
		btnInsertUsed.setTooltip(new Tooltip("Click here to move the above selected entry up one position"));

// Set up the Move Up button. It moves the current selected TableView artifact
// up one
// position. Link the button to the action method and define the tool tip to
// help the user
// understand the user interface.
		setupButtonUI(btnMoveUpUsed, "Arial", 14, 70, Pos.BASELINE_CENTER, 700 + xOffset, 280 + yOffset);
		btnMoveUpUsed.setOnAction((event) -> {
			moveUsedArtifactUp();
		});
		btnMoveUpUsed.setTooltip(new Tooltip("Click here to move the above selected entry up one position"));

// Set up the Move Dn button. It moves the current selected TabkeView artifact
// down one
// position. Link the button to the action method and define the tool tip to
// help the user
// understand the user interface.
		setupButtonUI(btnMoveDnUsed, "Arial", 14, 70, Pos.BASELINE_CENTER, 790 + xOffset, 280 + yOffset);
		btnMoveDnUsed.setOnAction((event) -> {
			moveUsedArtifactDn();
		});
		btnMoveDnUsed.setTooltip(new Tooltip("Click here to move the above selected entry down one position"));

// Set up the Delete button. It removes the current selected TabkeView artifact.
// Link the
// button to the action method and define the tool tip to help the user
// understand the
// user interface.
		setupButtonUI(btnDeleteUsed, "Arial", 14, 70, Pos.BASELINE_CENTER, 875 + xOffset, 280 + yOffset);
		btnDeleteUsed.setOnAction((event) -> {
			deleteUsedArtifact();
		});
		btnDeleteUsed.setTooltip(new Tooltip("Click here to delete the above selected entry"));

// This is the title for the Artifacts Produced Table View List and collection
// of buttons
...
// This ComboBox specifies an available option. No action is performed when one
// is selected
// The user must press the insert button to cause the selected item to be added
// to the list
...
// produced
		...
		btnInsertProduced.setOnAction((event) -> {
...
// Add all of the GUI elements to this window so they can be displayed
	...
	/**********
	 * Private local method to initialize the standard fields for a JavaFX Label
	 * object
	 * 
	...
	/**********
	 * Private local method to initialize the standard fields for a JavaFX ComboBox
	 * object
	 * 
	 ...
	 * @param e A flag that specific if the TextArea is editable
	 */
	private void setupTableViewProducedUI(TableView<ECUsedItem> a, double w, double h, double x, double y, boolean e) {
		a.setMinWidth(w);
		a.setMaxWidth(w);
		a.setMinHeight(h);
		a.setMaxHeight(h);
	...
	/**********
	 * This method is one of a pair that is used to ensure that the list of all
	 * possible options for a list agrees with the list of available to select items
	 * and the list of selected items. This method returns true only if the
	 * specified String is NOT in the specified list of Strings. Otherwise, it
	 * returns false;
	 * 
...
// If the second parameter is *not* in the list, return true
	...
	/**********
	 * This method is the second of a pair that is used to ensure that the list of
	 * all possible options for a list agrees with the list of available to select
	 * items and the list of selected items. This method returns false only if the
	 * specified String is NOT in the specified list of Strings. Otherwise, it
	 * returns true;
	 * 
	 * @param strList The specified list of Strings
...
// If the second parameter is *not* in the list, return true
	...
	/**********
	 * This private method is called four times each time this tab is activated. It
	 * rebuilds the Options list from the Tab's list and duplicates it into the
	 * Available list. The method then checks each item in the Selected list to
	 * ensure it is still in the Options list. If it isn't, it is removed. Finally
	 * the Available list is created by removing all of the Selected items from the
	 * stating state where it contains all of the Options.
	 * 
	 ...
	 * @param available The list of all of the possible options that have not yet
	 *                  been selected
	 */
...
// 2. Examine each Selected list item to ensure it is in STILL in the Option
// list
...
// 3. Construct the Available List by removing all Selected items from the
// Option's list
...
// remove is really important!
	...
	/**********
	 * Handle the editTheEntry Button action by doing the super class function and
	 * then the Tab specific work
	 */
...
// Re-populate the lists
		repopulateAList(artifactsAvailable, artifactsAvailableForUsed, artifactsUsedSelected, artifactsUsedAvailable);
		repopulateAList(artifactsAvailable, artifactsAvailableForProduced, artifactsProducedSelected,
				artifactsProducedAvailable);
...
// Set up the produced Table View
		ObservableList<ECUsedItem> producedList = theProducedDetailList.getItems();
		producedList.clear();
		for (int ndx = 0; ndx < artifactsProducedSelected.size(); ndx++)
			producedList.add(new ECUsedItem(artifactsProducedSelected.get(ndx)));
		theProducedDetailList.getSelectionModel().clearAndSelect(0);
	}

	/**********
	 * Handle the saveTheEntry Button action by doing the super class function and
	 * then the Tab specific work
	 */
	...
	/**********
	 * Handle the AddTheEntryToTheBottom Button action by doing the super class
	 * function and then the Tab specific work
	 */
	...
	/**********
	 * Handle the AddTheEntryAbove Button action by doing the super class function
	 * and then the Tab specific work
	 */
	...
	/**********
	 * Handle the AddTheEntryBelow Button action by doing the super class function
	 * and then the Tab specific work
	 */
	...
	/**********
	 * Handle the clearTheEntry Button action by doing the super class function and
	 * then the Tab specific work
	 */
	...
	/**********
	 * This methods is used to establish the Used Table View list based on a list of
	 * Strings
	 * 
...
// Return the result
	...
	/**********
	 * This methods is used to establish the Produced Table View list based on a
	 * list of Strings
	 * 
	 * @param src The list of names to be used to create the table view list
	 * @return The table view list
	 */
	private ObservableList<ECUsedItem> establishProducedDetailList(ObservableList<String> src) {
// Fetch the table view list from the right most GUI Table View
		ObservableList<ECUsedItem> result = theProducedDetailList.getItems();
...
// Return the result
	...
	/**********
	 * This method is called by the UserInterface each time the user changes tabs
	 * and selects this Task tab
	 * 
...
// If, when the lists are constructed, no Effort Category has been selected,
// choose the
...
// We are done with the repopulation, so user events can now be processed
	...
	private boolean isInTheList(String str, ListItem lst) {
		...
		for (int ndx = 0; ndx < oList.size(); ndx++)
			if (oList.get(ndx).getListItemName().contentEquals(str))
				return true;
		return false;
	}
	...
	private boolean artifactIsNotAvailable(String str) {
		for (int ndx = 0; ndx < artifactsAvailable.size(); ndx++)
			if (str.contentEquals(artifactsAvailable.get(ndx)))
				return false;
		return true;
	}
	...
	private boolean artifactHasNotBeenUsedSelected(String str) {
		for (int ndx = 0; ndx < artifactsUsedSelected.size(); ndx++)
			if (str.contentEquals(artifactsUsedSelected.get(ndx)))
				return false;
		return true;
	}
	...
	private boolean artifactHasNotBeenProducedSelected(String str) {
		for (int ndx = 0; ndx < artifactsProducedSelected.size(); ndx++)
			if (str.contentEquals(artifactsProducedSelected.get(ndx)))
				return false;
		return true;
	}
	...
	private void clearTheRightMostLists() {
		...
		ObservableList<ECUsedItem> emptyECProducedItem = FXCollections.observableArrayList();
	...
	/**********
	 * Given a list of Effort Categories, this method produces a list for the
	 * ComboBox
	 * 
	...
	/**********
	 * Given the parameters, this method produces all of the Table Views and
	 * ComboBoxes needed
	 * 
...
// See if there was a previously selected Effort Category in the
// currentSelectedEffortCategory
		if (ndx < 0) {
// If the previously used Effort Category is no longer there, do not select an
// item
			clearTheRightMostLists();
...
// Build the list of potential artifacts from the currently selected Effort
// Category and definitions
			ObservableList<String> temp = ecs.theEffortCategoryDetailList.get(selectedNdx - 1)
...
// Rebuild the list of current selected item give the previous set of selected
// items
...
// Rebuild the list of current available items by removing those items that have
// been selected
...
// There was not a selected EffortCategory item, so clear the tab's right most
// third
			clearTheRightMostLists();
	}

	/**********
	 * This event handler is used to select an EffortCategory and populate the right
	 * third of the tab accordingly
	 */
...
// If no item has been selected, select the first item in the list, None
// Selected
		if (currentSelectedEffortCategory == null)
...
// Signal that we are done updating the lists and it is okay to process user
// input
		updatingTheList = false;
	}

	/**********
	 * The method is used to shift an items back from the Selected List to the
	 * Available List when a selected items is deleted.
	 * 
	 * @param str  The name to be shifted back from the Selected List to the
	 *             Available List
	 ...
	 */
	private void shiftBack(ComboBox<String> acombo, String str, ObservableList<String> mstr, ObservableList<String> avl,
			ObservableList<String> sel) {
// We will scan through the Master List which gives the order of items that the
// user specified
// The Master List should be the union of the Available List and the Selected
// List
...
// The Master list elements are either in the Available List or the Selected
// List. The
// order of the items in the Available and Selected List match the order in the
// Master List.
// We go through the Master list starting at the beginning and examine each item
// for the
// that list. We know it will be in one or the other
		for (int masterIndex = 0; masterIndex < theSize; masterIndex++)
// If the item from the master list matched the specified String, we know it
// must be in
...
// With the shift back performed, we reset the comboBox and select what we just
// put back
		acombo.setItems(avl);
	...
	/**********
	 * Shift the specified String from the Available List to the Selected List and
	 * then set up the Table View to reflect this action
	 * 
	 ...
	 */
	private void shift(TableView<ECUsedItem> tvl, String str, ObservableList<String> src, ObservableList<String> dst) {
...
// Fetch the list of items from the Table View GUI item and change it to agree
// with the
// new Selected List
		ObservableList<ECUsedItem> de = tvl.getItems(); // Fetch the list
		de.clear(); // Clear it
...
// Insert the new list into the Table View, Scroll to the point of insertion,
// and select it
		tvl.setItems(de);
	...
	/**********
	 * This method insert a new row into the specified Table View and updated the
	 * associated lists accordingly
	 */
	private void insertAction(TableView<ECUsedItem> tvl, ComboBox<String> cb, ObservableList<String> src,
...
// Check to be sure there is an item in the ComboBox to insert
		if (src.size() < 1) {
			Alert alert = new Alert(AlertType.WARNING, "Insert is only valid when there's an item to insert!",
					ButtonType.OK);
			alert.showAndWait();
			updatingTheList = false;
			return;
...
// We are done updating the lists
	 ...
	 */
	private void moveUpAction(TableView<ECUsedItem> tvl) {
...
// If no row is selected, give a warning and ignore the request
		if (theIndex == -1) {
			Alert alert = new Alert(AlertType.WARNING, "Move Up is only valid when a row has been selected!",
					ButtonType.OK);
			alert.showAndWait();
			return;
		}

// If the selected row is the top most row, it is not possible to move it up, so
// give an
...
// Given a valid and proper index, produce an observable list from the TableView
// and swap
// the selected item with the one above it.
		ObservableList<ECUsedItem> de = tvl.getItems();
...
// Done updating the list
	 ...
	 */
	private void moveDnAction(TableView<ECUsedItem> tvl) {
// Fetch the index of the selected row in the TableView
		int theIndex = tvl.getSelectionModel().getSelectedIndex();

// If no row is selected, give a warning and ignore the request
		if (theIndex == -1) {
			Alert alert = new Alert(AlertType.WARNING, "Move Dn is only valid when a row has been selected!",
					ButtonType.OK);
			alert.showAndWait();
			return;
		}

// If the selected row is the bottom row, it is not possible to move it down, so
// give an
// appropriate warning and ignore the request
		ObservableList<ECUsedItem> de = tvl.getItems();
...
// Done updating the list
	...
	/**********
	 * This method deletes the currently selected item in the TableView list and
	 * updates all the associated lists.
	 */
	private void deleteAction(TableView<ECUsedItem> tvl, ComboBox<String> cb, ObservableList<String> mstr,
			ObservableList<String> sel, ObservableList<String> avl) {
// Fetch the index of the selected row in the TableView
		int theIndex = tvl.getSelectionModel().getSelectedIndex();

// If no row is selected, give a warning and ignore the request
		if (theIndex == -1) {
			Alert alert = new Alert(AlertType.WARNING, "Delete is only valid when a row has been selected!",
					ButtonType.OK);
			alert.showAndWait();
			return;
		}
...
// Done updating the list
	...
	/**********
	 * This event handler processes the Insert button for the used artifact set of
	 * buttons
	 */
	...
	/**********
	 * This event handler processes the Move Up button for the used artifact set of
	 * buttons
	 */
	...
	/**********
	 * This event handler processes the Move Dn button for the used artifact set of
	 * buttons
	 */
	...
	/**********
	 * This event handler processes the Delete button for the used artifact set of
	 * buttons
	 */
	private void deleteUsedArtifact() {
		deleteAction(theUsedDetailList, theUsedArtifact, artifactsAvailable, artifactsUsedAvailable,
				artifactsUsedSelected);
	}

	/**********
	 * This event handler processes the Insert button for the produced artifact set
	 * of buttons
	 */
	...
	/**********
	 * This event handler processes the Move Up button for the produced artifact set
	 * of buttons
	 */
	...
	/**********
	 * This event handler processes the Movbe Dn button for the produced artifact
	 * set of buttons
	 */
	...
	/**********
	 * This event handler processes the Delete button for the produced artifact set
	 * of buttons
	 */
	private void deleteProducedArtifact() {
		deleteAction(theProducedDetailList, theProducedArtifact, artifactsAvailable, artifactsProducedAvailable,
				artifactsProducedSelected);

	}

	/**********
	 * This nested protected Class is used to populate the rightmost TableView GUI.
	 * It is just a list of String objects. The methods for this class are totally
	 * obvious, so I'll not document them.
	 * 
	 * @author Lynn Robert Carter
	 *
	 */
	...
	/**********
	 * This nested protected Class is used to define a specific Effort Category with
	 * a set of four lists. Each of the four is just a list of String objects, but
	 * each time a specific Effort Category is selected, all four of the Selected
	 * lists and all of the dependent lists must be set up. The methods for this
	 * class are totally obvious, so I'll not document them.
	 * 
	 * @author Lynn Robert Carter
	 *
	 */
...
// Make shallow copies of each of the four lists
	 ...
	 * 
	 * private String display(ObservableList <String> src) { String result = "[";
	 * for (int ndx = 0; ndx < src.size(); ndx++) result += src.get(ndx) + (ndx <
	 * src.size()-1 ? "," : ""); return result + "]"; }
	 */
}